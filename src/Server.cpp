...
/*   Updated: 2025/01/13 23:19:39 by gbruscan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Server.hpp"
#include <arpa/inet.h>
#include <cstdio>  // Ajout pour perror
#include <cstdlib> // Ajout pour EXIT_FAILURE
#include <cstring>
#include <ctime>
#include <fcntl.h>
#include <iomanip>
...
void Server::acceptNewClient()
{
	...
	std::cout << "Nouveau client connecté : " << newClient->getHostname() << std::endl;
}
...
void Server::handleClientMessage(int fd)
{
	...
	if (message.find("USER ") != std::string::npos)
	{
		...
		std::string welcomeMsg = 
			":localhost 001 " + client->getNickname() + " :Welcome to the IRC server\r\n" +
			":localhost 002 " + client->getNickname() + " :Your host is localhost, running version 1.0\r\n" +
			":localhost 003 " + client->getNickname() + " :This server was created today\r\n" +
			":localhost 004 " + client->getNickname() + " localhost 1.0 i\r\n";

		send(fd, welcomeMsg.c_str(), welcomeMsg.size(), 0);
	}
	std::string modeMsg = ":" + client->getNickname() + " MODE " + client->getNickname() + " +i\r\n";
