...
Channel::Channel()
    : userLimit(-1), inviteOnly(false), topicRestricted(false)
{
}
...
{
    if (isUserInChannel(channelName, user.fd)) //mesonge est ce que le user est dans le channel
...
{
    _channels[channelName].topic = topic; // in commande handler check op or topic for all
}

void ChannelManager::setUserLimit(std::string channelName, int userLimit)
{
    _channels[channelName].userLimit = userLimit; //check if user is in the channel and is op
}

void ChannelManager::setPassword(std::string channelName, std::string password)
{
    _channels[channelName].password = password; //check if user is in the channel and is op
}

void ChannelManager::setInviteOnly(std::string channelName, bool inviteOnly)
{
    _channels[channelName].inviteOnly = inviteOnly; //check if user is in the channel and is op
}

void ChannelManager::setTopicRestricted(std::string channelName, bool topicRestricted)
{
    _channels[channelName].topicRestricted = topicRestricted; //check if user is in the channel and is op
}
...
{
    return (_channels[channelName].users.find(fd) != _channels[channelName].users.end());
}

bool ChannelManager::isOperator(std::string channelName, int fd)
{
    return (_channels[channelName].operators.find(fd) != _channels[channelName].operators.end());
}
...
{
    return (_channels[channelName].userLimit != -1 && _channels[channelName].users.size() >= (unsigned long long)_channels[channelName].userLimit);
}
