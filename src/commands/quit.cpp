...
void Server::doQuit(const TokenisedCommand &cmd, int fd)
{
    ...
    for (std::map<std::string, Channel *>::const_iterator it = inChannel.begin(); it != inChannel.end(); ++it) {
        std::cout << "fd = "<< fd << "    et channel name = " << it->first << std::endl;
        if (it->second && _channels.isUserInChannel(it->first, fd)) {
            quitMsg = ":" + _clients.getClient(fd)->nickname + "!" + _clients.getClientname(fd) + "!@GLMRC QUIT " + ":" + cmd.getArguments()[0] + "\n";
